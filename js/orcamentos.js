...
let rtOrcModeloExternoArquivo = '';
function rtOrcArquivoModeloAssociado(){
  try {
    const cfg = (typeof carregarConfiguracoes === 'function') ? carregarConfiguracoes() : {};
    const arquivo = String(cfg?.modelosArquivosDocumentos?.orcamento || 'modelo-orcamento.html').trim().replace(/^\/+/, '');
    return (!arquivo || arquivo.includes('..') || /[\\]/.test(arquivo)) ? 'modelo-orcamento.html' : arquivo;
  } catch(_) { return 'modelo-orcamento.html'; }
}
function rtOrcLimparCacheModeloExterno(){ rtOrcModeloExternoCache=''; rtOrcModeloExternoArquivo=''; }
window.rtOrcLimparCacheModeloExterno = rtOrcLimparCacheModeloExterno;
async function rtOrcCarregarModeloExterno(){
  ...
  try {
    ...
    rtOrcModeloExternoArquivo = arquivo;
    console.error(`Modelo HTML ${arquivo} não pôde ser carregado.`, e);
  }
}
...
async function gerarPdfOrcamento(o){
  ...
  const nomeArquivoPdf = `orcamento-${String(o.numero || numeroProximoOrcamento(o.data_evento) || 'PREVIEW')}.pdf`;
  const html = `<!doctype html><html><head><meta charset="utf-8"><title>${rtOrcEscape(nomeArquivoPdf.replace(/\.pdf$/i, ''))}</title><style>
