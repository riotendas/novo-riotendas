...
function rtFinRenderExtratoSalvo() {
  ...
  tbody.innerHTML = linhasRender.map(l => {
    ...
    let selecionado = rtFinEventoKeyLinha(l);
    let sugestao = null;
    if (l.sugestao_json) {
      try { sugestao = JSON.parse(l.sugestao_json); } catch (e) {}
    }
    if (!sugestao && tipo === "entrada" && String(l.status || "pendente") === "pendente") sugestao = rtFinSugerirEventoParaExtrato(itemSugestao);
    if (!selecionado.replace("||", "") && sugestao && sugestao.evento_id && sugestao.tipo_pagamento && Number(sugestao.confianca || 0) >= 60) selecionado = `${sugestao.evento_id}||${sugestao.tipo_pagamento}`;
    // Se ainda não havia seleção gravada, deixa a melhor opção provável já selecionada.
    const melhorOpcao = opcoes.find(o => (o.key === selecionado || o.legacy_key === selecionado)) || opcoes[0];
    if (!selecionado.replace("||", "") && melhorOpcao && Number(melhorOpcao._score || 0) >= 45) selecionado = melhorOpcao.key;
    const opcoesHtml = opcoes.map(o => `<option value="${rtFinEscapeHtml(o.key)}" ${(o.key === selecionado || o.legacy_key === selecionado) ? "selected" : ""}>${rtFinEscapeHtml(o.label)}</option>`).join("");
    ...
    const assoc = podeVincular ? `
      ...
      <div class="financeiro-registro fin-ext-auto-status" data-ext-auto-status="${rtFinEscapeHtml(id)}"><span>${selecionado ? "✅ Seleção pronta — troca salva automaticamente" : "Selecione uma opção para salvar automaticamente"}</span></div>
      ${sugestao && sugestao.evento_id ? `<div class="financeiro-registro"><span>💡 Sugestão: ${rtFinEscapeHtml(sugestao.cliente || "")} · aviso ${rtFinDataBR(sugestao.data_aviso || "")} · ${rtFinEscapeHtml(sugestao.tipo_pagamento || "")} · ${rtFinMoeda(sugestao.valor_esperado || 0)} (${sugestao.confianca || 0}%)</span></div>` : ""}
    ...
    return `<tr class="financeiro-status-${l.status || "pendente"}">
      ...
      <td><span class="financeiro-status-badge">${rtFinStatusLinhaExtrato(l)}</span>${l.cliente_nome ? `<div class="financeiro-registro fin-ext-assoc-resumo" title="${rtFinEscapeHtml(l.cliente_nome)} · ${rtFinDataBR(l.evento_data)} · ${rtFinEscapeHtml(l.tipo_pagamento || "")}"><span>${rtFinDataBR(l.evento_data)} | ${rtFinEscapeHtml(l.tipo_pagamento || "")} | ${rtFinEscapeHtml(l.cliente_nome)}</span></div>` : ""}</td>
      <td>${assoc}</td>
      <td><input class="fin-ext-obs" data-ext-obs="${rtFinEscapeHtml(id)}" value="${rtFinEscapeHtml(l.observacao || "")}" placeholder="observação"></td>
      <td class="financeiro-acoes">
        ${podeVincular ? `<button type="button" class="btn-mini" data-ext-salvar-assoc="${rtFinEscapeHtml(id)}">Salvar associação</button>` : ""}
        <button type="button" class="btn-mini btn-outline" data-ext-marcar="rendimento" data-ext-id="${rtFinEscapeHtml(id)}">Rendimento</button>
        <button type="button" class="btn-mini btn-outline" data-ext-marcar="outro" data-ext-id="${rtFinEscapeHtml(id)}">Outro</button>
        <button type="button" class="btn-mini btn-outline" data-ext-marcar="pendente" data-ext-id="${rtFinEscapeHtml(id)}">Pendente</button>
        <button type="button" class="btn-mini btn-outline" data-ext-marcar="ignorado" data-ext-id="${rtFinEscapeHtml(id)}">Ignorar</button>
      </td>
    </tr>`;
  }).join("");
  ...
  tbody.querySelectorAll("select[data-ext-assoc]").forEach(sel => sel.addEventListener("change", async () => {
    const id = sel.dataset.extAssoc;
    const status = document.querySelector(`[data-ext-auto-status="${CSS.escape(id)}"] span`);
    if (!sel.value) return;
    if (status) status.textContent = "Salvando associação...";
    const ok = await rtFinSalvarAssociacaoExtrato(id, true);
    if (ok && status) status.textContent = "✅ Associação salva automaticamente";
  }));
...
async function rtFinSalvarAssociacaoExtrato(id, silencioso = false) {
  const sel = document.querySelector(`select[data-ext-assoc="${CSS.escape(id)}"]`);
  const obs = document.querySelector(`input[data-ext-obs="${CSS.escape(id)}"]`)?.value || "";
  if (!sel || !sel.value) {
    if (!silencioso) alert("Escolha o evento/pagamento para associar.");
    return false;
  }
  const op = rtFinOpcoesPagamentoEventos().find(o => o.key === sel.value);
  if (!op) return false;
  const ok = await rtFinAtualizarLinhaExtrato(id, {
    status: "associado",
    cliente_nome: op.cliente,
    evento_id: op.evento_id || null,
    evento_data: op.data_evento || null,
    tipo_pagamento: op.tipo_pagamento,
    valor_associado: op.valor,
    observacao: obs
  });
  return ok;
}
